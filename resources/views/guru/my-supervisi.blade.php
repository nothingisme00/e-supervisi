...
<div class="w-full lg:w-11/12 xl:w-5/6 mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-gray-50 dark:bg-gray-900/30 rounded-2xl p-5 md:p-6 mb-6">
        <!-- Header Card -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow mb-4">
            <div class="px-4 py-3.5 md:px-6 md:py-4 lg:px-7 lg:py-5">
                <div class="flex flex-row items-center justify-between gap-3 md:gap-5 w-full">
                    ...
                    <div class="flex items-center gap-2 md:gap-4 flex-shrink min-w-0">
                        <div class="w-10 h-10 md:w-14 md:h-14 lg:w-16 lg:h-16 bg-gradient-to-r from-emerald-600 to-teal-600 dark:from-emerald-500 dark:to-teal-500 rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
                        ...
                        <div class="min-w-0 flex-1">
                            <h3 class="text-base md:text-xl lg:text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">Supervisi Saya</h3>
                            <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400 hidden sm:block">Daftar semua supervisi pembelajaran Anda • {{ $mySupervisi->count() }} supervisi</p>
                        </div>
                    </div>
        ...
        @if($mySupervisi->count() > 0)
            <div class="grid grid-cols-1 gap-4">
                @foreach($mySupervisi as $item)
                    <!-- Supervisi Card -->
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all duration-200 overflow-hidden">
                        <!-- Card Header -->
                        ...
                        <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700/30 border-t border-gray-200 dark:border-gray-700 flex items-center justify-end gap-2">
                            @if($item->status == 'draft')
                                ...
                                <form action="{{ route('guru.supervisi.delete', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus supervisi ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-4 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-lg transition-all shadow-sm">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-8 text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Belum Ada Supervisi</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Anda belum memiliki supervisi apapun. Mulai dengan membuat supervisi baru.</p>
                <button onclick="openSupervisiModal()" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-semibold rounded-lg transition-all shadow-md hover:shadow-lg">
